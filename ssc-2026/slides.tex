...
\begin{itemize}
    ...
    \item Linear control systems \citep{willsky1976}
     \vspace{3mm}
    \item Supervised learning (AI) for clinical prediction + decision-making \citep{fengEtal2022}
     \vspace{3mm}
    \item Remote health monitoring (BP, EKG, etc.) \citep{kishore2026}
     \vspace{3mm}
    \item Anomalies in sensor networks \citep{shiryaev2010, xie2013}
     \vspace{3mm}
    \item Structural change in financial models \citep{chu1996}
\end{itemize}
...
Pollak--Siegmund conditional average delay:
  $$
\displaystyle
      \CAD(D) := \sup_{\nu\ge 1}\E_{P,\nu,Q}\!\left[(D - \nu) \,\middle|\, D \ge \nu\right]$$
  \begin{itemize}
...
A \textit{test supermartingale} ($M_t$) `does not grow' under $\mathcal{P}$: 
...
\vspace{3mm}
Powerful tool for sequential analysis \citep{ramdasEtal23, grunwaldEtal24}
  \begin{center}
...
\begin{frame}{From a \TSM\ to a detector: Shiryaev--Roberts (\ARL\ control)}
  ...
  Given any \textit{test supermartingale} $(\Mt)$ for $\mathcal{P}$ with increments $\Lt := \Mt / M_{t-1}$, define
  \[
  ...
  betting against $P$ and compute $\Rt$ as the \emph{running sum} of your wealth. \citep{shin_2023} \\
  \medskip
  \vspace{3mm}
...
\begin{frame}{Picture: the Shiryaev--Roberts procedure}
  \centering
  \includegraphics[width=0.7\linewidth]{figures/tsm_sr_intuition.png}\\
  \vspace{-0.5em}
...
\begin{frame}{Nonparametric likelihood ratio TSM}
    ...
    $$ \prod_{i=1}^t [1 + \lambda_i (X_i - \eta_i)]$$
    is a TSM when $\mathbb{E}[X_t | \mathcal{F}_{t-1}] \leq \eta_t$ and $\lambda_t \in [0,1/\eta]$ for all $t$:\\
    \vspace{3mm}
    \textbf{Features:}
    \begin{itemize}
        \item Finite-sample validity over nonparametric $\mathcal{P}$---all non-negative distributions with (conditional) mean less than $\eta_t$. 
        \item Includes: Bernoulli, Beta, Poisson, truncated normal, contingency tables, all finite-populations; IID, Markov, VAR data
        \item Optimal betting $\implies$ efficiency \citep{waudby-smithRamdas24, spertus24}
        \item Handle additional nuisance parameters via maximization \citep{spertus2024sequential}
    \end{itemize}
\end{frame}


% ---------- Multivariate combining ----------
\begin{frame}{Joint detectors via meta-analysis of bets}
  \alert{Goal}: detector fires if \textit{any} stream changes \\ 
  \medskip
  Let $(M_{tk})$ be a marginal \TSM\ for stream $k$, with increments $\Lambda_{tk}$.
  \medskip
  \begin{block}{Four ways to combine:}
  ...
  \end{block}
  \medskip
  Asymptotically, $M_t^u \geq M_t^m$ \citep{cover1991}, but $M_t^u$ is hard to compute\\
  \medskip
...
\begin{frame}{Localization via spending allowance}
    ...
    \begin{itemize}
        ...
        \item $\gamma_{tk}$ with fixed weights $\implies$ prioritize ``more important'' streams 
        \item $\gamma_{tk}$ as Bayesian posterior $\implies$ model alarms, focus on streams likely to change next
    \end{itemize}
\end{frame}
...
\begin{frame}{Simulation snapshot: regret of different combining strategies}
[Fix this!]
  \begin{columns}[T,onlytextwidth]
    \column{0.6\textwidth}
      \centering
...
\begin{frame}{What is the ``state of control''?}
  ...
  (e.g.\ absolute thresholds or hospital-aligned signals).
\end{frame}
...
\begin{frame}{Sparse vs.\ dense change: where each combiner wins}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
      \textbf{Sparse change.}\\
      $\bs{\mu}_1 - \bs{\mu}_0 = \Delta \cdot \bs{e}_{(j)}$.\\
      One marginal \TSM\ grows.\\
      UP is fine (beats max)
    \column{0.5\textwidth}
      \textbf{Dense change.}\\
      $\bs{\mu}_1 - \bs{\mu}_0 = (\Delta/\sqrt{K})\cdot \bm{1}$.\\
      Every marginal \TSM\ grows.\\
      Product compounds; UP loses.
  \end{columns}
  % \vspace{0.4em}
  % \centering
