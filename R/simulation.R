...
#   N        = integer horizon per replication
...
run_simulation <- function(detector, tsm, dgp, n_rep = 200, N = 500, K = 1L, combiner = NULL, weights = NULL, seed = 1L, log = FALSE) {
  ...
  for (di in seq_along(detector)) {
    for (gi in seq_along(dgp)) {
      ...
      fa <- mean(is.finite(stop_times) & (!is.finite(nu) | stop_times <= nu))
      arl <- mean(pmin(stop_times, N + 1))
      add <- if (is.finite(nu)) mean(pmax(stop_times - nu, 0), na.rm = TRUE) else NA_real_
